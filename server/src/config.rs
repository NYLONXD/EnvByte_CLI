...
impl Config {
    pub fn from_env() -> Result<Self, String> {
        let environment = env::var("ENVBYTE_ENV").unwrap_or_else(|_| "development".to_string());
        let database_url = required("DATABASE_URL")?;
        let jwt_secret = required("ENVBYTE_JWT_SECRET")?;
        if jwt_secret.len() < 32 {
            return Err("ENVBYTE_JWT_SECRET must be at least 32 characters".to_string());
        }
        let email = match env::var("ENVBYTE_EMAIL_MODE")
            ...
            .as_str()
        {
            ...
            value => return Err(format!("Unsupported ENVBYTE_EMAIL_MODE: {value}")),
        };
        if environment == "production" && matches!(email, EmailConfig::Log) {
            return Err("ENVBYTE_EMAIL_MODE=log is forbidden in production".to_string());
        }
        Ok(Self {
            environment,
            bind_addr: parse("ENVBYTE_BIND", SocketAddr::from(([0, 0, 0, 0], 3030)))?,
            database_url,
            jwt_secret,
            jwt_issuer: env::var("ENVBYTE_JWT_ISSUER").unwrap_or_else(|_| "envbyte".to_string()),
            access_token_ttl_seconds: parse("ENVBYTE_ACCESS_TTL_SECONDS", 3600)?,
            refresh_token_ttl_days: parse("ENVBYTE_REFRESH_TTL_DAYS", 30)?,
            invite_ttl_hours: parse("ENVBYTE_INVITE_TTL_HOURS", 24)?,
            email_verification_ttl_hours: parse("ENVBYTE_EMAIL_VERIFICATION_TTL_HOURS", 2)?,
